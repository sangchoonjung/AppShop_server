...
router.post('/ConsumerPendingRequest', async (req, resp) => {
    try {
        ...
        const discountPerOneUnit =
            targetProduct.DiscountRate /
            (targetProduct.ProductQuantity - targetProduct.MinimumQuantity);
        //개당할인율
        const FinalPrice =
            targetProduct.SoldQuantity + unit > targetProduct.MinimumQuantity
                ? targetProduct.Price *
                  (1 -
                      (discountPerOneUnit / 100) *
                          (targetProduct.SoldQuantity + unit))
                : targetProduct.FinalPrice;
        //팔린 수량 + 구매 수량 > 최소 수량이면 계산식
        await Product.findOneAndUpdate(
            { SKU: productId },
            { SoldQuantity: rstQun, FinalPrice: FinalPrice }
        );
        // 수량 계산 끝, 응답 보냄
        resp.status(200).json({
            result: true,
